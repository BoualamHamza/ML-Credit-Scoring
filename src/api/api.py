...
def load_full_dataset() -> Optional[pd.DataFrame]:
    """Load full dataset for client ID lookup"""
    try:
        if FEATURES_PATH.exists():
            ...
            logger.warning(f"Dataset file not found: {FEATURES_PATH}")
            return None
    except Exception as e:
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    global model, feature_names, full_dataset, model_type, model_loaded
    ...
    if result[0] is not None:
        ...
        if full_dataset is None:
            logger.warning("Full dataset not loaded, client ID lookup will not work")
